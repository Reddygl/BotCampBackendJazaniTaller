...
namespace Jazani.Infraestructura.Cores.Persistences
{
    public abstract class CrudRepository<T, ID> : ICrudRepository<T, ID> where T : class
    {
        ...
        public virtual async Task<T?> FindByIdAndKeyAsync(ID id, ID key)
        { 
            return await _dbContext.Set<T>().FindAsync(id, key);
        }
